...
export default function RelatedSarees({ currentSareeId }: { currentSareeId: string }) {
  ...
  useEffect(() => {
    async function loadSarees() {
      try {
        ...
        // Filter out current saree and get 4 random sarees
        const filteredSarees = data
          .filter((saree) => saree.id !== currentSareeId)
          .sort(() => 0.5 - Math.random())
          .slice(0, 4)
        setSarees(filteredSarees)
      } catch (error) {
        console.error("Failed to load related sarees:", error)
      } finally {
        setLoading(false)
      }
    }
